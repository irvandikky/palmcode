<div class="py-12">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6 lg:p-8 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center justify-between mb-4">
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search categories..." class="form-input rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-200 w-1/3">
                            ...
                            @forelse ($categories as $category)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="editCategory({{ $category->id }})" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600 mr-3">Edit</button>
                                        <button wire:click="deleteCategory({{ $category->id }})" wire:confirm="Are you sure you want to delete this category?"
    ...
    @if ($showCategoryFormModal)
        <div class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" style="display: block;">
            <div class="fixed inset-0 transform transition-all" wire:click="$set('showCategoryFormModal', false)">
                <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
            </div>

            <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl transform sm:w-full sm:mx-auto max-w-2xl" x-data="{ show: @entangle('showCategoryFormModal') }" x-init="$watch('show', value => {
                ...
                <div class="px-6 py-4">
                    ...
                    <div class="mt-4 text-sm text-gray-600 dark:text-gray-400">
                        <livewire:forms.category-form :category-id="$editingCategoryId" @category-saved="$refresh" />
                    </div>
                </div>
            </div>
        </div>
